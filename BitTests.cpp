...
static Bit xor_reduce(const Bit& acc, const Bit& value)
{
    return acc ^ value;
}

static bool is_true_bit(const Bit& b)
{
    return b.GetValue();
}

static BitSequence* MakeBitSequence(const char* bits)
{
...
static void AssertBits(const Sequence<Bit>& seq, const char* expected)
{
